...
import psutil
import GPUtil
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QLineEdit, QPushButton, QTextBrowser, QProgressBar, QLabel, QSlider, QHBoxLayout
from PyQt5.QtCore import QTimer, Qt, QRunnable, QThreadPool, pyqtSlot, QObject, pyqtSignal, QUrl
...
def load_image_from_url(url):
    try:
        ...
        print(f"Error: Cannot identify image from URL {url} - {str(e)}")
        return None

def get_image_urls_from_html(html, base_url):
...
def process_image(image, image_url):
    ...
    if crops:
        ...
        for i, crop_probs in enumerate(probs):
            ...
            # HTML structure with a table for scores
            html = f"""
            ...
            <table style="border: 1px solid {border_color}; margin-top: 5px; width: 100%;">
                <tr><th>Category</th><th>Score</th></tr>
            """
            for category, probability in sorted_scores:
                html += f"<tr><td>{category.capitalize()}</td><td>{probability * 100:.2f}%</td></tr>"
            html += "</table>"
...
class MainWindow(QMainWindow):
    ...
    def on_page_load_finished(self):
        if self.web_page.html_ready:
            html = self.web_page.html
            base_url = self.web_view.url().toString()
            image_urls = get_image_urls_from_html(html, base_url)
            if image_urls:
                self.results_browser.append(f"Found {len(image_urls)} images on {base_url}")
    ...
    def start_workers(self):
        while not self.queue.empty() and self.executor.activeThreadCount() < self.threads_slider.value():
