...
model_detection = torch.hub.load('ultralyticsyolov5', 'yolov5s')

# Load the classification model (ViT)
device = cuda if torch.cuda.is_available() else cpu
model_classification, preprocess = clip.load(ViT-B32, device=device)
...
def process_image(image, image_url)
    ...
    if crops
        ...
        text_inputs = torch.cat([clip.tokenize(fa photo of a {c}) for c in categories]).to(device)
        ...
        for i, crop_probs in enumerate(probs)
            ...
            st.write(fnImage probably includes a {sorted_scores[0][0].capitalize()} 
...
if st.button(Test URL)
    if webpage_url
        ...
        if image_urls
            st.write(fFound {len(image_urls)} images on {webpage_url})
