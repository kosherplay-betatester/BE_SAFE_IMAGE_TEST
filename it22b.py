    ...
    try:
        async with session.get(url) as response:
            if response.status == 200:
                ...
                print(f"Error: Cannot fetch image from URL {url}")
                return None
...
async def load_image_from_url(url):
    async with aiohttp.ClientSession() as session:
        image = await fetch_image(session, url)
        return image
...
class Worker(QRunnable):
    ...
    def run(self):
        ...
        image = loop.run_until_complete(load_image_from_url(self.image_url))
        result = process_image(image, self.image_url)
        self.signals.result.emit(result)
        gc.collect()
        torch.cuda.empty_cache()  # Clear CUDA cache if GPU is used
